...
export const addEmote: CommandBody = {
  ...
  callback: async (msg: Message<boolean>) => {
    try {
      ...
      const newEmote: GuildEmoji = await msg.guild.emojis.create({
        attachment: url,
        name,
      });
  ...
  slashCallback: async (interaction: ChatInputCommandInteraction) => {
    try {
      const url: string = interaction.options.get(EmoteOptions.URL)
        ?.value as string;
      const name: string = interaction.options.get(EmoteOptions.NAME)
        ?.value as string;
      const newEmote: GuildEmoji = await interaction.guild.emojis.create({
        attachment: url,
        name,
      });
