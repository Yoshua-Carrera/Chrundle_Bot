...
class ChrundleBot {
  ...
  initializeBot(): void {
    const options = this.unpackYargs().argv as {
    ...
    this.client.on(
      ...
      async (msg: Message<boolean>) => {
        ...
        if (options.privateCommands && msg.author.id !== process.env.PRIVATE)
          return;
        ...
        const commandMatch = this.chrundleDtoService.isCommandMatch(
          msg,
          this.slashCommandsService.loadCommands(),
        );
    ...
    this.client.on(
      ...
      (interaction: Interaction<CacheType>) => {
        ...
        if (
          options.privateCommands &&
          interaction.user.id !== process.env.PRIVATE
        )
          return;
        const slashCommandMatch = this.chrundleDtoService.isSlashCommandMatch(
          interaction,
          this.slashCommandsService.loadCommands(),
        );
...
const chrundleBot = new ChrundleBot(
  new ChrundleDtoService(),
  new SlashCommandsService(),
);
